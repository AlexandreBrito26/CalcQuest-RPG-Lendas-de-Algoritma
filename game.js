...
rpg.getMaxHp = function () {
  ...
  if (this.inventory.includes("r_omega")) baseHp *= 50.0;
...
rpg.getAtk = function () {
  ...
  if (this.inventory.includes("r_omega")) baseAtk *= 50.0;
...
rpg.getAtk = function () {
  let base = _origGetAtk.call(this);
  if (this.coopActive && this.coopPartner) {
    base = Math.floor(base * (1 + (this.coopPartner.atkBonus || 0)));
  }
  return base;
};
...
const _runeGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  let base = _runeGetAtk();
  ...
  if (this.getRuneEffect("berserker") && this.heroHp > 0) {
    const lostPct = 1 - (this.heroHp / Math.max(1, this.getMaxHp()));
    const bonus = Math.min(0.6, Math.floor(lostPct / 0.1) * 0.06);
    base = Math.floor(base * (1 + bonus));
  }
  if (this.getRuneEffect("momentum") && this.runeKillStack > 0)
    base = Math.floor(base * (1 + Math.min(this.runeKillStack, 50) * 0.02));
  return base;
};

const _runeGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _runeGetMaxHp();
...
const _runeCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() {
  let base = _runeCrit();
  if (this.getRuneEffect("omnivore")) base = Math.min(0.95, base + 0.25);
  return base;
};

const _runeDodge = rpg.getDodgeChance.bind(rpg);
rpg.getDodgeChance = function() {
  let base = _runeDodge();
  if (this.getRuneEffect("omnivore")) base = Math.min(0.90, base + 0.25);
  return base;
};

// Rune mirror reflect on magic
const _runeDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  if (this.getRuneEffect("mirror") && atkType === "mag" && Math.random() < 0.25 && this.monster && this.monster.hp > 0) {
    const reflectDmg = Math.floor(baseDmg * 0.5);
    this.monster.hp = Math.max(0, this.monster.hp - reflectDmg);
    this.showDamage("🪞 -" + formatNumber(reflectDmg), "dmg-effective");
    if (this.monster.hp <= 0) { this.killMonster(); return; }
  }
  _runeDealDmg(baseDmg, atkType, isUltimate);
  if (this.getRuneEffect("vampiric") && this.monster && baseDmg > 0) {
    const steal = Math.floor(baseDmg * 0.15);
    if (steal > 0) { this.heroHp = Math.min(this.getMaxHp(), this.heroHp + steal); this.showDamage("🩸 +" + formatNumber(steal), "heal"); }
  }
};

// Rune regen on monster turn
const _runeMonsterAtk = rpg.executeMonsterAttack.bind(rpg);
rpg.executeMonsterAttack = function() {
...
const _runeCooldown = rpg.startCooldown.bind(rpg);
rpg.startCooldown = function(id) {
  if (this.getRuneEffect("timeloop") && Math.random() < 0.20) {
    showToast("⏰ Loop Temporal!", 900); return;
  }
  _runeCooldown(id);
};

// Rune storm: 20% free magic
const _runeUseSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  _runeUseSkill(id);
  if (id === "mag" && this.getRuneEffect("storm") && Math.random() < 0.20) {
    this.skills.mag.timer = false;
    const btn = document.getElementById("btn-mag"); if (btn) btn.disabled = false;
    const cd  = document.getElementById("cd-mag");  if (cd)  cd.style.width = "0%";
    showToast("⚡ Tempestade! Grátis!", 900);
  }
};

// Rune momentum: kill stack
const _runeMomentumKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
...
const _achGetAtk2 = rpg.getAtk.bind(rpg);
rpg.getAtk = function() { return Math.floor(_achGetAtk2() * (1 + (this.permAtkBonus||0) + (this.permAllBonus||0))); };
const _achGetMaxHp2 = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() { return Math.floor(_achGetMaxHp2() * (1 + (this.permAllBonus||0))); };
const _achCrit2 = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() { return Math.min(0.95, _achCrit2() + (this.permCritBonus||0) + (this.permAllBonus||0)*0.5); };
const _achDodge2 = rpg.getDodgeChance.bind(rpg);
rpg.getDodgeChance = function() { return Math.min(0.90, _achDodge2() + (this.permDodgeBonus||0)); };

// Track gold earned
const _achKillMonster = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  const goldBefore = this.gold;
  _achKillMonster();
  const earned = this.gold - goldBefore;
  if (earned > 0) {
    this.totalGoldEarned = (this.totalGoldEarned||0) + earned;
...
const _waveKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  if (this.waveActive) {
    this.waveNumber++;
    if (this.waveNumber - 1 > this.bestWave) {
...
const _waveDiePatch = rpg.die.bind(rpg);
rpg.die = function() {
  if (this.waveActive) {
    this.waveActive = false;
    showToast("💀 Onda " + (this.waveNumber-1) + " — Recorde: " + this.bestWave, 5000);
  }
  _waveDiePatch();
};

const _waveSpawnPatch = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _waveSpawnPatch();
  if (this.waveActive && this.monster) {
    const wm = 1 + (this.waveNumber - 1) * 0.15;
    this.monster.hp    = Math.floor(this.monster.hp    * wm);
    this.monster.maxHp = Math.floor(this.monster.maxHp * wm);
    this.monster.dmg   = Math.floor(this.monster.dmg   * wm);
    if (this.waveNumber % 10 === 0) {
      this.monster.name = "⚠ " + this.monster.name + " [ÉLITE]";
      this.monster.hp   *= 3; this.monster.maxHp *= 3; this.monster.dmg *= 2;
    }
    this.updateHpBars();
  }
};
...
const _ddKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  if (this.dailyDungeonActive) {
    this.dailyDungeonFloor++;
    if (this.dailyDungeonFloor >= this.DAILY_DUNGEON_FLOORS) {
      this.dailyDungeonActive = false;
      this.dailyDungeonDone   = true;
      this.dungeonsCleared    = (this.dungeonsCleared||0) + 1;
      const goldR = Math.floor(this.level * 500);
      const potR  = this.DAILY_DUNGEON_FLOORS * 3;
      this.gold   += goldR; this.potions += potR;
      localStorage.setItem("rpg_dd_done", "true");
      localStorage.setItem("rpg_dungeons", this.dungeonsCleared);
      showToast("🏆 Dungeon Completa! +" + formatNumber(goldR) + " 💰 +" + potR + " 🧪", 5000);
      this.checkAchievements(); this.save();
    } else {
      showToast("🏰 Andar " + this.dailyDungeonFloor + " / " + this.DAILY_DUNGEON_FLOORS, 1500);
    }
  }
  _ddKillPatch();
};

const _ddSpawnPatch = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _ddSpawnPatch();
  if (this.dailyDungeonActive && this.monster && this.ddMods) {
...
// Grimoire patches
const _griAddFury = rpg.addFury.bind(rpg);
rpg.addFury = function(amount) { _griAddFury(this.grimoireFuryBoost ? Math.floor(amount * 1.5) : amount); };

const _griDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  const actualDmg = (atkType === "mag" && this.grimoireArcaneSurge) ? Math.floor(baseDmg * 1.5) : baseDmg;
  _griDealDmg(actualDmg, atkType, isUltimate);
  if (this.grimoireLifeSteal && baseDmg > 0) {
    const steal = Math.floor(baseDmg * 0.08);
    if (steal > 0) { this.heroHp = Math.min(this.getMaxHp(), this.heroHp + steal); }
  }
};
...
const _titUpdateUI = rpg.updateUI.bind(rpg);
rpg.updateUI = function() {
  _titUpdateUI();
  const title = this.getTitle();
  const el = document.getElementById("hero-title-badge");
  if (el) el.textContent = title.name[this.lang];
  this.checkAchievements();
};
...
const _chSpawnPatch = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _chSpawnPatch();
  if (this.challengeActive && this.monster) {
    this.activeChallenges.forEach(id => {
      const ch = this.CHALLENGES.find(c=>c.id===id);
      if (ch && ch.speedMult) this.monster.spd = Math.max(100, Math.floor(this.monster.spd / ch.speedMult));
    });
    if (this.activeChallenges.some(id => ["ch_fragile","ch_hardcore"].includes(id))) {
      // HP mod aplicado via getMaxHp override temporário — já aplicado pelo challengeActive flag
    }
  }
};

const _chUseSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  if (this.challengeActive) {
    if (id==="heal" && this.activeChallenges.includes("ch_no_heal"))  { showToast("❌ Sem Cura (Desafio)!"); return; }
    if (id==="def"  && (this.activeChallenges.includes("ch_no_def") || this.activeChallenges.includes("ch_hardcore")))  { showToast("❌ Sem Defesa (Desafio)!"); return; }
  }
  _chUseSkill(id);
};

// Apply challenge kill bonus
const _chKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  if (this.challengeActive && this.activeChallenges.length > 0) {
    const boost = this.getChallengeMultiplier() - 1;
    const bonusGold = Math.floor(this.level * 100 * boost);
    if (bonusGold > 0) { this.gold += bonusGold; }
  }
  _chKillPatch();
};

const _chEndBattle = rpg.endBattle.bind(rpg);
rpg.endBattle = function() {
...
// fragile — HP override
const _chGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _chGetMaxHp();
  if (this.challengeActive && (this.activeChallenges.includes("ch_fragile") || this.activeChallenges.includes("ch_hardcore")))
...
const _v18Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_unlocked_runes",  JSON.stringify(this.unlockedRunes));
  localStorage.setItem("rpg_runes",           JSON.stringify(this.equippedRunes));
  localStorage.setItem("rpg_achievements",    JSON.stringify(this.achievementsClaimed));
  localStorage.setItem("rpg_grimoire",        JSON.stringify(this.grimoire));
  localStorage.setItem("rpg_best_wave",       this.bestWave);
  localStorage.setItem("rpg_dungeons",        this.dungeonsCleared);
  localStorage.setItem("rpg_total_gold",      this.totalGoldEarned);
  localStorage.setItem("rpg_dd_date",         this.dailyDungeonDate);
  localStorage.setItem("rpg_dd_done",         this.dailyDungeonDone);
  _v18Save();
};

const _v18Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.unlockedRunes      = JSON.parse(localStorage.getItem("rpg_unlocked_runes")  || "[]");
  this.equippedRunes      = JSON.parse(localStorage.getItem("rpg_runes")            || "[]");
  this.achievementsClaimed= JSON.parse(localStorage.getItem("rpg_achievements")    || "[]");
  this.grimoire           = JSON.parse(localStorage.getItem("rpg_grimoire")         || "[]");
  this.bestWave           = parseInt(localStorage.getItem("rpg_best_wave")          || "0");
  this.dungeonsCleared    = parseInt(localStorage.getItem("rpg_dungeons")           || "0");
  this.totalGoldEarned    = parseInt(localStorage.getItem("rpg_total_gold")         || "0");
  this.dailyDungeonDate   = localStorage.getItem("rpg_dd_date")                     || "";
  this.dailyDungeonDone   = localStorage.getItem("rpg_dd_done") === "true";
  this.grimoirePhoenix    = false; this.grimoireFuryBoost = false;
  this.grimoireLifeSteal  = false; this.grimoireComboMax  = 20;
  this.grimoireArcaneSurge= false;
  this._applyAchievements();
  this._applyGrimoire();
  _v18Init();
};
...
const _talentLevelUp = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
...
const _talentDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  if (atkType === "atk" && this.talentDoubleStrike && Math.random() < 0.20 && this.monster && this.monster.hp > 0) {
    setTimeout(() => {
      if (this.monster && this.monster.hp > 0) {
        this.showDamage("⚔⚔ DUPLO!", "dmg-effective");
        _talentDealDmg(Math.floor(baseDmg * 0.7), "atk", false);
      }
    }, 300);
  }
  const boostedDmg = (isUltimate && this.talentUltimateMult > 1)
    ? Math.floor(baseDmg * this.talentUltimateMult)
    : baseDmg;
  _talentDealDmg(boostedDmg, atkType, isUltimate);
};

// Talent: low HP ATK boost (via getAtk patch)
const _talentGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  let base = _talentGetAtk();
  if (this.talentLowHpAtk && this.heroHp > 0 && this.heroHp / Math.max(1, this.getMaxHp()) < 0.5)
    base = Math.floor(base * 1.20);
  return base;
};

// Talent: kill heal
const _talentKillHealPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _talentKillHealPatch();
...
const _talentMonsterAtkPatch = rpg.executeMonsterAttack.bind(rpg);
rpg.executeMonsterAttack = function() {
  // regen per turn
  if (this.talentRegen) {
  ...
  _talentMonsterAtkPatch();
  // spirit shield
  if (this.talentSpiritShield && this.heroHp <= 0 && !this._spiritUsed) {
    this._spiritUsed = true;
    this.heroHp = 1;
    this.updateHpBars();
    this.showDamage("🛡 ESPÍRITO!", "dmg-parry");
  }
};

// Talent: invulnerability
const _talentConfirmBattle = rpg.confirmBattle.bind(rpg);
rpg.confirmBattle = function() {
  this._invulnUsed = false;
  this._spiritUsed = false;
  _talentConfirmBattle();
};

// Talent: heal boost
const _talentUseSkillPatch = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  if (id === "heal" && this.talentHealBoost !== 1) {
    // override heal amount temporarily
    this._talentHealActive = true;
  }
  _talentUseSkillPatch(id);
  this._talentHealActive = false;
};

// Patch getMaxHp for talent potion heal (intercept heal amount)
const _talentHealPatch = rpg.useSkill.bind(rpg);

// CD reduce
const _talentCdPatch = rpg.startCooldown.bind(rpg);
rpg.startCooldown = function(id) {
  if (this.talentCdReduce > 0) {
    const skill = this.skills[id];
    if (skill) {
      const origCd = skill.cd;
      skill.cd = Math.floor(origCd * (1 - this.talentCdReduce));
      _talentCdPatch(id);
      skill.cd = origCd;
      return;
    }
...
const _forgeGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  const base = _forgeGetAtk();
  const bonus = this.getForgeBonus(this.eqWeapon);
...
const _wandererKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _wandererKillPatch();
  this.trySpawnWanderer();
  // Decrease temp buffs
...
const _wandGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() { return Math.floor(_wandGetAtk() * (1 + (this.tempAtkBoost||0))); };

const _wandGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() { return Math.min(0.95, _wandGetCrit() + (this.tempCritBoost||0)); };

const _wandGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() { return Math.floor(_wandGetMaxHp() * (1 + (this.tempHpBoost||0))); };

const _wandCooldown = rpg.startCooldown.bind(rpg);
rpg.startCooldown = function(id) {
  if (this.tempCdBoost > 0) {
    const skill = this.skills[id];
    if (skill) {
      const orig = skill.cd;
      skill.cd = Math.floor(orig * (1 - this.tempCdBoost));
      _wandCooldown(id);
      skill.cd = orig;
      return;
    }
...
const _battleEventSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _battleEventSpawn();
  if (this.inCombat && !this.isBossFight) this.checkBattleEvent();
};
...
const _mathKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _mathKillPatch();
  if (this.mathBattleActive && this.inCombat && this.kills % 3 === 0) {
    setTimeout(() => this.triggerMathQuestion(), 1500);
  }
};

// Math bonus doubles damage
const _mathDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  const dmg = this.mathBonusActive ? Math.floor(baseDmg * 2) : baseDmg;
  _mathDealDmg(dmg, atkType, isUltimate);
};
...
const _cursedGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  let base = _cursedGetAtk();
  const r = this.getCursedRelic();
  if (!r) return base;
  if (r.atkBuff) base = Math.floor(base * r.atkBuff);
  if (r.allBuff) base = Math.floor(base * r.allBuff);
  return base;
};

const _cursedGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _cursedGetMaxHp();
  const r = this.getCursedRelic();
  if (!r) return base;
  if (r.hpPenalty) base = Math.floor(base * r.hpPenalty);
  if (r.allBuff)   base = Math.floor(base * r.allBuff);
  return base;
};

const _cursedGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() {
  let base = _cursedGetCrit();
  const r = this.getCursedRelic();
  if (r && r.critBuff) base = Math.min(0.99, base + r.critBuff);
  return base;
};

// Self-harm curse
const _cursedDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  const r = this.getCursedRelic();
  if (r && r.selfHarm && Math.random() < r.selfHarm) {
    const selfDmg = Math.floor(this.getMaxHp() * 0.08);
    this.heroHp = Math.max(1, this.heroHp - selfDmg);
    this.showDamage("💀 -" + formatNumber(selfDmg), "dmg-player");
    this.updateHpBars();
  }
  _cursedDealDmg(baseDmg, atkType, isUltimate);
};

// No defense curse
const _cursedUseSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  const r = this.getCursedRelic();
  if (r && r.noDef && id === "def") { showToast("💀 Maldição: Sem Defesa!"); return; }
  if (r && r.noPotions && id === "heal") { showToast("💀 Maldição: Sem Poções!"); return; }
  _cursedUseSkill(id);
};

// Gold mult for cursed relics
const _cursedKillMonster = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _cursedKillMonster();
  const r = this.getCursedRelic();
  if (r && (r.goldMult || r.gainsMult)) {
    const mult = (r.goldMult || r.gainsMult || 1) - 1;
    const bonus = Math.floor(this.level * 50 * mult);
...
// Apply gem stats
const _gemGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  let base = _gemGetAtk();
  base = Math.floor(base * (1 + this.getGemBonusFor("atk", this.eqWeapon) + this.getGemBonusFor("all", this.eqWeapon)));
  base = Math.floor(base * (1 + this.getGemBonusFor("atk", this.eqArmor) + this.getGemBonusFor("all", this.eqArmor)));
  return base;
};

const _gemGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _gemGetMaxHp();
  base = Math.floor(base * (1 + this.getGemBonusFor("hp", this.eqWeapon) + this.getGemBonusFor("all", this.eqWeapon)));
  base = Math.floor(base * (1 + this.getGemBonusFor("hp", this.eqArmor) + this.getGemBonusFor("all", this.eqArmor)));
  return base;
};

const _gemGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() {
  return Math.min(0.99, _gemGetCrit() + this.getGemBonusFor("crit", this.eqWeapon) + this.getGemBonusFor("crit", this.eqArmor));
};

const _gemGetDodge = rpg.getDodgeChance.bind(rpg);
rpg.getDodgeChance = function() {
  return Math.min(0.90, _gemGetDodge() + this.getGemBonusFor("dodge", this.eqWeapon) + this.getGemBonusFor("dodge", this.eqArmor));
};

const _gemAddFury = rpg.addFury.bind(rpg);
rpg.addFury = function(amount) {
  const bonus = this.getGemBonusFor("fury", this.eqWeapon) + this.getGemBonusFor("fury", this.eqArmor);
  _gemAddFury(Math.floor(amount * (1 + bonus)));
};
...
const _honorKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _honorKillPatch();
  let honorGain = 0;
  if (this.isBossFight) honorGain += 10;
  if (["chaos","realmgod"].includes(this.difficulty)) honorGain += 5;
  if (this.waveActive && this.waveNumber > 1) honorGain += Math.floor(this.waveNumber * 0.2);
  if (this.challengeActive) honorGain += 3;
  if (honorGain > 0) this.addHonor(honorGain);
};
...
const _v19Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_talent_pts",     this.talentPoints);
  localStorage.setItem("rpg_talents",        JSON.stringify(this.unlockedTalents));
  localStorage.setItem("rpg_forge",          JSON.stringify(this.forgeUpgrades));
...
const _v19Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.talentPoints       = parseInt(localStorage.getItem("rpg_talent_pts") || "0");
  this.unlockedTalents    = JSON.parse(localStorage.getItem("rpg_talents")        || "[]");
  this.forgeUpgrades      = JSON.parse(localStorage.getItem("rpg_forge")          || "{}");
  this.equippedGems       = JSON.parse(localStorage.getItem("rpg_gems")           || "{}");
  this.ownedGems          = JSON.parse(localStorage.getItem("rpg_owned_gems")     || "[]");
  this.equippedCursedRelic= localStorage.getItem("rpg_cursed_relic") || null;
  this.unlockedCursedRelics=JSON.parse(localStorage.getItem("rpg_cursed_relics")  || "[]");
  this.honor              = parseInt(localStorage.getItem("rpg_honor")             || "0");
  this.purchasedHonor     = JSON.parse(localStorage.getItem("rpg_honor_shop")     || "[]");
  // Apply talent tree
  this.talentDoubleStrike=false; this.talentFuryBoost=0; this.talentCritMult=2;
  this.talentArmorPierce=0; this.talentLowHpAtk=false; this.talentKillHeal=false;
  this.talentUltimateMult=1; this.talentCounterMult=2; this.talentDefAbsorb=false;
  this.talentRegen=false; this.talentSpiritShield=false; this.talentInvulnerability=false;
  this.talentHealBoost=1; this.talentPotionHeal=0.40; this.talentCdReduce=0; this.talentFreePot=false;
  this._applyTalents();
  // Apply honor shop
  this.purchasedHonor.forEach(id => {
    const item = this.HONOR_SHOP.find(h=>h.id===id);
    if (item && item.apply) item.apply(this);
  });
  this.tempAtkBoost=0; this.tempCritBoost=0; this.tempCdBoost=0; this.tempHpBoost=0;
  this._tempCritBattles=0; this._tempCdBattles=0; this._tempHpBattles=0;
  this.nextRewardDouble=false; this.bossGoldBoost=false;
  this.wandererKills=0; this.wandererActive=false; this.wandererOffers=[];
  this.mathBattleActive=false; this.mathBonusActive=false;
  _v19Init();
};
...
rpg.MAP_REGIONS = [
  {
    id: "r_ruins",      x: 45, y: 80, name: {pt:"Ruínas do Despertar", en:"Ruins of Awakening"},
    reqBoss: 0,  reqLvl: 1,   color: "#71717a", icon: "⚔",
    desc: {pt:"O ponto de origem. Onde tudo começou.", en:"The origin point. Where it all began."},
    theme: "t_ruins", chapter: "intro", biome: "medieval"
  },
  {
    id: "r_swamp",      x: 25, y: 65, name: {pt:"Pântano Tóxico",       en:"Toxic Swamp"},
    ...
    theme: "t_swamp", chapter: null, biome: "swamp"
  },
  {
    id: "r_forest",     x: 60, y: 60, name: {pt:"Floresta Antiga",       en:"Ancient Forest"},
    ...
    theme: "t_forest", chapter: "act2", biome: "forest"
  },
  {
    ...
    theme: "t_volcano", chapter: "act3", biome: "volcano"
  },
  {
    id: "r_cave",       x: 30, y: 40, name: {pt:"Caverna do Esquecimento",en:"Cave of Forgetting"},
    reqBoss: 1,  reqLvl: 35,  color: "#6d28d9", icon: "🕳",
    desc: {pt:"A memória RAM corrompida. Ecos de bosses derrotados.", en:"Corrupted RAM memory. Echoes of defeated bosses."},
    theme: "t_cave", chapter: null, biome: "cave"
  },
  {
    id: "r_astral",     x: 55, y: 30, name: {pt:"Plano Astral",          en:"Astral Plane"},
    reqBoss: 2,  reqLvl: 65,  color: "#7c3aed", icon: "🌌",
    desc: {pt:"Entre dimensões. A gravidade é opcional.", en:"Between dimensions. Gravity is optional."},
    theme: "t_astral", chapter: null, biome: "astral"
  },
  {
    id: "r_cyber",      x: 20, y: 25, name: {pt:"Fenda Digital",         en:"Digital Rift"},
    ...
    theme: "t_underground", chapter: "dlc_cyber", biome: "cyber"
  },
  {
    id: "r_void",       x: 50, y: 15, name: {pt:"O Vazio",               en:"The Void"},
    reqBoss: 4,  reqLvl: 150, color: "#18181b", icon: "🌀",
    desc: {pt:"Nada existe aqui. Apenas poder puro.", en:"Nothing exists here. Only pure power."},
    theme: "t_void", chapter: "dlc_ultimate", biome: "void"
  },
  {
    id: "r_matrix",     x: 75, y: 20, name: {pt:"Realidade Base",        en:"Base Reality"},
    ...
    theme: "t_matrix", chapter: "dlc_creator", biome: "matrix"
  },
  {
    id: "r_quantum",    x: 50, y: 5,  name: {pt:"Núcleo Quântico",       en:"Quantum Core"},
    ...
    theme: "t_quantum", chapter: "dlc_quantum", biome: "quantum"
  },
  {
    id: "r_end",        x: 50, y: -5, name: {pt:"O Fim de Tudo",         en:"The End of All"},
    ...
    theme: "t_end", chapter: "interlude_1", biome: "end"
  },
];
...
rpg.renderMap = function() {
  ...
  const svgNodes = this.MAP_REGIONS.map(region => {
    ...
    const cx = region.x * 2.8; // scale to ~280px wide
    const cy = Math.max(5, region.y) * 2.2; // scale to ~220px tall, flip y
    const r = isCurrent ? 10 : 7;
    const fill = unlocked ? region.color : "#27272a";
    const stroke = isCurrent ? "#fff" : (discovered ? region.color : "#52525b");
    const opacity = unlocked ? 1 : 0.4;
    return `<g class="cursor-pointer" onclick="rpg.travelToRegion('${region.id}')" opacity="${opacity}">
      <circle cx="${cx}" cy="${220 - cy}" r="${r+4}" fill="${fill}22" />
      <circle cx="${cx}" cy="${220 - cy}" r="${r}" fill="${fill}" stroke="${stroke}" stroke-width="${isCurrent ? 3 : 1.5}" />
      ${isCurrent ? `<circle cx="${cx}" cy="${220 - cy}" r="${r+7}" fill="none" stroke="${stroke}" stroke-width="1" stroke-dasharray="3 2" opacity="0.7"/>` : ""}
      <text x="${cx}" y="${220 - cy + 4}" text-anchor="middle" font-size="8" fill="white" font-weight="bold" opacity="${unlocked ? 1 : 0.4}">${region.icon}</text>
      <text x="${cx}" y="${220 - cy + r + 12}" text-anchor="middle" font-size="6" fill="${unlocked ? "#e4e4e7" : "#52525b"}" font-weight="bold">${region.name[this.lang].split(" ")[0]}</text>
    </g>`;
  }).join("");

  // Connection lines between adjacent regions
  const connections = [
    ["r_ruins","r_swamp"], ["r_ruins","r_forest"], ["r_ruins","r_cave"],
    ["r_swamp","r_cave"],  ["r_forest","r_volcano"],["r_forest","r_astral"],
    ["r_cave","r_cyber"],  ["r_astral","r_void"],   ["r_volcano","r_astral"],
    ["r_cyber","r_void"],  ["r_void","r_matrix"],   ["r_matrix","r_quantum"],
    ["r_quantum","r_end"],
  ];
  const svgLines = connections.map(([a, b]) => {
    ...
    if (!ra || !rb) return "";
    const x1 = ra.x * 2.8, y1 = 220 - Math.max(5,ra.y)*2.2;
    const x2 = rb.x * 2.8, y2 = 220 - Math.max(5,rb.y)*2.2;
    const bothUnlocked = this.mapDiscovered.includes(a) || this.mapDiscovered.includes(b);
    return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${bothUnlocked ? "#3f3f46" : "#27272a"}" stroke-width="1" stroke-dasharray="4 2" opacity="${bothUnlocked ? 0.6 : 0.2}" />`;
  }).join("");
  ...
  el.innerHTML = `
    <div class="bg-zinc-950/80 border border-zinc-800 rounded-xl p-3 mb-3">
      <div class="flex items-center gap-2">
        ...
        <div>
          <p class="text-xs font-black text-zinc-200">${current.name[this.lang]}</p>
          <p class="text-[9px] text-zinc-500 leading-tight">${current.desc[this.lang]}</p>
        </div>
      </div>
    </div>
    <div class="bg-zinc-950/80 border border-zinc-800 rounded-xl overflow-hidden mb-3">
      <svg viewBox="0 0 280 230" width="100%" height="220" style="background:radial-gradient(ellipse at 50% 100%,#1c1917,#09090b)">
    ...
    <div class="grid grid-cols-2 gap-1.5 max-h-40 overflow-y-auto hide-scrollbar">
      ${this.MAP_REGIONS.map(region => {
        const unlocked = this.level >= region.reqLvl && this.bossKills >= region.reqBoss;
        const isCurrent = this.currentRegion === region.id;
        return `<button onclick="rpg.travelToRegion('${region.id}')"
          class="text-left p-2 rounded-xl border ${isCurrent ? "border-white/30 bg-white/5" : unlocked ? "border-zinc-800 bg-zinc-950/60 hover:border-zinc-700" : "border-zinc-800/30 bg-zinc-950/30 opacity-40 cursor-not-allowed"} transition-colors">
          <p class="text-[9px] font-black ${unlocked ? "text-zinc-200" : "text-zinc-600"} leading-tight">${region.icon} ${region.name[this.lang]}</p>
          <p class="text-[7px] text-zinc-600 mt-0.5">Lvl ${region.reqLvl} · Boss ${region.reqBoss}</p>
        </button>`;
      }).join("")}
    </div>`;
  lucide.createIcons();
...
const _ncKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _ncKillPatch();
  if (this.isBossFight) setTimeout(() => this.checkNarrativeChoice(), 2000);
};
...
// Track ultimate uses
const _npcUseSkillPatch = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  _npcUseSkillPatch(id);
  if (id === "atk" && this.fury >= 100 - 1) {
    this.ultimateUses = (this.ultimateUses||0) + 1;
    localStorage.setItem("rpg_ult_uses", this.ultimateUses);
  }
};

// Track hardcore wins
const _npcEndBattlePatch = rpg.endBattle.bind(rpg);
rpg.endBattle = function() {
  if (this.challengeActive && this.activeChallenges.includes("ch_hardcore") && this.monster && this.monster.hp <= 0) {
    this.hardcoreWins = (this.hardcoreWins||0) + 1;
    localStorage.setItem("rpg_hardcore_wins", this.hardcoreWins);
  }
  _npcEndBattlePatch();
};
...
const _bossPartsSpawnPatch = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _bossPartsSpawnPatch();
  this.BOSS_PARTS_ACTIVE = false;
  this.bossPartHP = {};
  this.targetedPart = null;
  const el = document.getElementById("boss-parts-ui");
  if (el) el.innerHTML = "";
  if (this.isBossFight) setTimeout(() => this.initBossParts(), 300);
};

// Patch dealDamageToMonster para aplicar dano às partes
const _bossPartsDmgPatch = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  if (this.BOSS_PARTS_ACTIVE && this.monster) {
    this.damageBossPart(baseDmg);
  }
  _bossPartsDmgPatch(baseDmg, atkType, isUltimate);
};
...
const _v20Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_map",          JSON.stringify(this.mapDiscovered));
  localStorage.setItem("rpg_current_region", this.currentRegion || "r_ruins");
  localStorage.setItem("rpg_narrative",    JSON.stringify(this.narrativeChoices));
  localStorage.setItem("rpg_npc_quests",   JSON.stringify(this.npcQuestsDone));
  localStorage.setItem("rpg_ult_uses",     this.ultimateUses);
  localStorage.setItem("rpg_hardcore_wins",this.hardcoreWins);
...
const _v20Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.mapDiscovered    = JSON.parse(localStorage.getItem("rpg_map")           || "[]");
  this.currentRegion    = localStorage.getItem("rpg_current_region")           || "r_ruins";
  this.narrativeChoices = JSON.parse(localStorage.getItem("rpg_narrative")     || "{}");
  this.npcQuestsDone    = JSON.parse(localStorage.getItem("rpg_npc_quests")    || "[]");
  this.ultimateUses     = parseInt(localStorage.getItem("rpg_ult_uses")        || "0");
  this.hardcoreWins     = parseInt(localStorage.getItem("rpg_hardcore_wins")   || "0");
  this.forgeDisco       = 0;
  this.BOSS_PARTS_ACTIVE= false; this.bossPartHP = {}; this.targetedPart = null;
  // Re-apply narrative choices
  Object.entries(this.narrativeChoices).forEach(([cid, optId]) => {
    const choice = this.NARRATIVE_CHOICES.find(c => c.id === cid);
    if (choice) {
      const opt = choice.options.find(o => o.id === optId);
      if (opt) opt.apply(this);
    }
  });
  // Re-apply NPC quest rewards
  this.npcQuestsDone.forEach(qid => {
    this.NPCS.forEach(npc => {
      const q = npc.quests.find(x => x.id === qid);
      if (q) q.apply(this);
    });
  });
  // Discover starting region
  if (!this.mapDiscovered.includes("r_ruins")) this.mapDiscovered.push("r_ruins");
  _v20Init();
};
...
const _chainUseSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  _chainUseSkill(id);
  if (this.inCombat && this.monster && this.monster.hp > 0) {
    setTimeout(() => this.checkSkillChain(id), 100);
  }
};
...
const _formGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  const base = _formGetAtk();
  const f = this.getFormation();
  return Math.floor(base * (f.mods.atk || 1));
};

const _formGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  const base = _formGetMaxHp();
  const f = this.getFormation();
  if (f.mods.hpFlat !== undefined) return f.mods.hpFlat;
  return Math.floor(base * (f.mods.hp || 1));
};

const _formGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() {
  const base = _formGetCrit();
  const f = this.getFormation();
  return Math.min(0.99, base + (f.mods.crit || 0));
};

const _formGetDodge = rpg.getDodgeChance.bind(rpg);
rpg.getDodgeChance = function() {
  const base = _formGetDodge();
  const f = this.getFormation();
  return Math.min(0.90, base + (f.mods.dodge || 0));
};

// Formation: magic mult + cd reduction
const _formDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  const f = this.getFormation();
  const dmg = (atkType === "mag" && f.mods.magMult) ? Math.floor(baseDmg * f.mods.magMult) : baseDmg;
  _formDealDmg(dmg, atkType, isUltimate);
};

const _formCooldown = rpg.startCooldown.bind(rpg);
rpg.startCooldown = function(id) {
  const f = this.getFormation();
  if (id === "mag" && f.mods.magCd) {
    const skill = this.skills.mag;
    const orig = skill.cd;
    skill.cd = Math.floor(orig * f.mods.magCd);
    _formCooldown(id);
    skill.cd = orig;
    return;
  }
...
const _procKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _procKillPatch();
  if (this.procDungeon && !this.isBossFight) {
    setTimeout(() => this.nextProcRoom(), 800);
  } else if (this.procDungeon && this.isBossFight) {
    setTimeout(() => this.completeProcDungeon(), 800);
  }
};
...
const _journalUpdateUI = rpg.updateUI.bind(rpg);
rpg.updateUI = function() {
  _journalUpdateUI();
...
// Karma events
const _karmaHealSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  _karmaHealSkill(id);
  if (id === "heal") this.addKarma(2);
  if (id === "def")  this.addKarma(1);
};

const _karmaKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _karmaKillPatch();
  // Dark karma bonus: more gold for dark players
  const kb = this.getKarmaBonus();
  if (kb.gold > 1) {
    const bonus = Math.floor(this.level * 20 * (kb.gold - 1));
    if (bonus > 0) this.gold += bonus;
  }
};
...
const _karmaGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  const base = _karmaGetAtk();
  return Math.floor(base * this.getKarmaBonus().atk);
};

const _karmaGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  const base = _karmaGetMaxHp();
  return Math.floor(base * this.getKarmaBonus().hp);
};

const _karmaGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() {
  return Math.min(0.99, _karmaGetCrit() + this.getKarmaBonus().crit);
};
...
const _partnerConfirmBattle = rpg.confirmBattle.bind(rpg);
rpg.confirmBattle = function() {
  _partnerConfirmBattle();
  if (this.partnerActive && this.aiPartner) {
    setTimeout(() => this.startAiPartnerCombat(), 1000);
  }
};
...
// Auto-poison spec
const _specDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  _specDealDmg(baseDmg, atkType, isUltimate);
  if (this.specAutoPoison && Math.random() < 0.30 && this.monster && this.monster.hp > 0) {
    this.applyStatus("poison");
  }
};
...
const _v21Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_journal",    JSON.stringify(this.heroJournal));
  localStorage.setItem("rpg_karma",      this.karma);
  localStorage.setItem("rpg_formation",  this.activeFormation);
  localStorage.setItem("rpg_ai_partner", JSON.stringify(this.aiPartner));
  localStorage.setItem("rpg_class_spec", this.classSpec || "");
  localStorage.setItem("rpg_best_dungeon", this.bestDungeon);
  _v21Save();
};

const _v21Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.heroJournal     = JSON.parse(localStorage.getItem("rpg_journal")     || "[]");
  this.karma           = parseInt(localStorage.getItem("rpg_karma")         || "50");
  this.activeFormation = localStorage.getItem("rpg_formation")              || "balanced";
  this.aiPartner       = JSON.parse(localStorage.getItem("rpg_ai_partner")  || "null");
  this.partnerActive   = !!this.aiPartner;
  this.classSpec       = localStorage.getItem("rpg_class_spec")             || null;
  this.bestDungeon     = parseInt(localStorage.getItem("rpg_best_dungeon")  || "0");
  this.skillChainSequence = []; this.skillChainCooldown = false;
  this.specAutoPoison  = false;
  this._applySpec();
  _v21Init();
};
...
const _formGetAtk = rpg.getAtk.bind(rpg);
rpg.getAtk = function() {
  let base = _formGetAtk();
  const f = this.getFormation();
  let mult = f.atkMult;
  if (f.chaosMode) mult = 0.5 + Math.random();
  return Math.floor(base * mult);
};

const _formGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _formGetMaxHp();
  const f = this.getFormation();
  let mult = f.hpMult;
  if (f.chaosMode) mult = 0.5 + Math.random();
  return Math.floor(base * mult);
...
rpg.getDodgeChance = function() {
  const f = this.getFormation();
  let base = _formGetDodge() * (f.dodgeMult || 1);
  if (f.critBonus) base = Math.min(0.90, base);
  return base;
};
...
rpg.getCritChance = function() {
  const f = this.getFormation();
  return Math.min(0.99, _formGetCrit() + (f.critBonus||0));
};

// Formation: arcane magic boost
const _formUseSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  if (id === "mag") {
    const f = this.getFormation();
    if (f.magicMult && f.magicMult > 1) {
      // Temporarily boost magic damage
      this._formMagicActive = true;
    }
  }
  _formUseSkill(id);
  this._formMagicActive = false;
};
...
rpg.startCooldown = function(id) {
  const f = this.getFormation();
  if (f.cdMult && f.cdMult !== 1.0) {
    const skill = this.skills[id];
    if (skill) {
      const orig = skill.cd;
      skill.cd = Math.floor(orig * f.cdMult);
      _formCooldown(id);
      skill.cd = orig;
      return;
    }
...
rpg.killMonster = function() {
  _procKillPatch();
  if (this.procDungeonActive && this.procDungeon) {
    this.procDungeonFloor++;
    const rem = this.procDungeon.floors - this.procDungeonFloor;
    if (rem <= 0) {
      setTimeout(() => this.completeProcDungeon(), 1000);
    } else {
      showToast("🏰 Andar " + this.procDungeonFloor + "/" + this.procDungeon.floors + " — " + rem + " restantes", 2000);
      setTimeout(() => this.enterNextProcRoom(), 1200);
    }
  }
};
...
rpg.useSkill = function(id) {
  _chainUseSkill(id);
  if (this.inCombat) this.checkSkillChain(id);
};
...
const _diaryUpdateUI = rpg.updateUI.bind(rpg);
rpg.updateUI = function() {
  _diaryUpdateUI();
  this.checkDiaryTriggers();
};
...
const _tournDmgPatch = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate) {
  _tournDmgPatch(baseDmg, atkType, isUltimate);
  if (this.tournamentActive) {
    this.tournamentDmgDone += baseDmg;
  }
};

// On boss kill during tournament
const _tournKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  if (this.tournamentActive && this.isBossFight) {
    this.tournamentActive = false;
    const week = this.getTournamentWeek();
    if (this.tournamentDmgDone > this.tournamentScore) {
      this.tournamentScore = this.tournamentDmgDone;
      localStorage.setItem("rpg_tourn_score", this.tournamentScore);
    }
    this.tournamentWeek = week;
    localStorage.setItem("rpg_tourn_week", week);
    const honor = Math.floor(Math.log10(Math.max(1, this.tournamentDmgDone)) * 10);
    this.addHonor(honor);
    showToast("🏆 Torneio! Dano: " + formatNumber(this.tournamentDmgDone) + " · +" + honor + " 🏅", 5000);
    this.save();
  }
  _tournKillPatch();
};
...
const _memKillPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _memKillPatch();
  if (this.isBossFight) this.checkSkillMemories();
};
...
rpg.save = function() {
  ...
  localStorage.setItem("rpg_diary",          JSON.stringify(this.heroDiary));
  localStorage.setItem("rpg_tourn_score",    this.tournamentScore);
  localStorage.setItem("rpg_tourn_week",     this.tournamentWeek);
  localStorage.setItem("rpg_skill_memories", JSON.stringify(this.skillMemories));
  localStorage.setItem("rpg_pd_stats",       JSON.stringify(this.procDungeonStats));
  _v21Save();
};
...
rpg.init = function() {
  this.activeFormation    = localStorage.getItem("rpg_formation")              || "balanced";
  this.heirlooms          = JSON.parse(localStorage.getItem("rpg_heirlooms")   || "[]");
  this.heroDiary          = JSON.parse(localStorage.getItem("rpg_diary")       || "[]");
  this.tournamentScore    = parseInt(localStorage.getItem("rpg_tourn_score")   || "0");
  this.tournamentWeek     = localStorage.getItem("rpg_tourn_week")             || "";
  this.skillMemories      = JSON.parse(localStorage.getItem("rpg_skill_memories") || "[]");
  this.procDungeonStats   = JSON.parse(localStorage.getItem("rpg_pd_stats")   || '{"cleared":0,"bestFloors":0}');
  this.skillChainBuffer   = [];
  this.skillChainCooldown = false;
  this.tournamentActive   = false;
  this.tournamentDmgDone  = 0;
  // Re-apply skill memories
  this.skillMemories.forEach(id => {
    const m = this.SKILL_MEMORIES.find(x=>x.id===id);
    if (m) m.apply(this);
  });
  _v21Init();
};
...
const _origSpawnMonster = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _origSpawnMonster();
  if (this.monster) {
    this.monster.element = this.monsterElements[this.monster.id] || "none";
    this.initBreakSystem(this.monster);
    this.statusEffects = {};
    this.renderStatusBar();
    this.renderShieldBar();
    this.battleLog = [];
    ...
    // Diálogo de start se boss
    if (this.isBossFight) {
      setTimeout(() => this.showBossDialogue("start"), 800);
    }
    this.addLog(`${this.monster.name} apareceu! ${this.elementChart[this.monster.element]?.emoji || ""}`, "text-red-400");
  }
};
...
const _origDealDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(baseDmg, atkType, isUltimate = false) {
  if (!this.monster || this.monster.hp <= 0) return;

  // Elemento do ataque (mage = fire, ataque físico = earth se warrior, etc.)
  const heroElem = this.classElements[this.eqClass] || "none";
  const atkElem = (atkType === "mag") ? heroElem : (atkType === "atk" ? heroElem : "none");
  const monsterElem = this.monster.element || "none";

  // Break State bonus
  const breakMult = (this.monster.inBreakState) ? 1.5 : 1.0;

  // Reação elemental antes do dano
  const reaction = this.checkElementReaction(atkElem);
  if (reaction) {
    this.showDamage(reaction.name[this.lang], "dmg-effective");
    if (reaction.heal) {
      const h = Math.floor(this.getMaxHp() * reaction.heal);
      this.heroHp = Math.min(this.getMaxHp(), this.heroHp + h);
      this.showDamage(`✨ +${formatNumber(h)}`, "heal");
    }
    ...
    baseDmg = Math.floor(baseDmg * (reaction.mult || 1));
    this.addLog(`⚗ Reação: ${reaction.name[this.lang]}`, "text-amber-300");
  }

  // Modificador elemental
  const { mult: elemMult, msg: elemMsg } = this.getElementDamageMultiplier(atkElem, monsterElem);
  baseDmg = Math.floor(baseDmg * elemMult * breakMult);

  if (elemMsg === "effective") setTimeout(() => this.showDamage(t("super_effective"), "dmg-effective"), 0);
  if (elemMsg === "resist")    setTimeout(() => this.showDamage(t("resistant"), "dmg-resistant"), 0);
  if (elemMsg === "absorb")  { this.showDamage("🛡 ABSORVIDO!", "dmg-parry"); this.updateHpBars(); return; }

  // Tentar quebrar escudo com fraqueza elemental
  if (elemMsg === "effective" && this.monster.shieldPoints > 0) {
    this.tryBreakShield();
  }

  // Tick de status (burn, bleed)
  this.tickStatusEffects();

  // Dano original
  _origDealDmg(baseDmg, atkType, isUltimate);

  // Break turn counter
  if (this.monster && this.monster.inBreakState) {
    this.monster.breakTurnsLeft--;
    if (this.monster.breakTurnsLeft <= 0) {
      this.monster.inBreakState = false;
      this.monster.shieldPoints = Math.max(1, Math.floor(this.monster.maxShieldPoints / 2));
      this.renderShieldBar();
    }
  }
  ...
  // Battle log
  if (this.monster && this.monster.hp > 0) {
    this.addLog(`${this.heroName} atacou ${this.monster.name}`, "text-blue-300");
  }
};
...
rpg.executeMonsterAttack = function() {
  ...
  // Curse: curas reduzidas (gerenciado via useSkill heal)
  // Poison tick no herói
  if (this.statusEffects.poison) {
    const poisonDmg = Math.floor(this.getMaxHp() * 0.03);
    this.heroHp = Math.max(1, this.heroHp - poisonDmg);
    this.showDamage(`☠️ -${formatNumber(poisonDmg)}`, "hero");
  ...
  // Diálogo de boss por threshold de HP
  if (this.isBossFight && this.monster) {
    const pct = this.monster.hp / this.monster.maxHp;
    if (!this._bossHalfTriggered && pct <= 0.5) {
      this._bossHalfTriggered = true;
      this.showBossDialogue("half");
    }
    if (!this._bossLowTriggered && pct <= 0.2) {
      this._bossLowTriggered = true;
      this.showBossDialogue("low");
    }
  }

  // Passar choque para _orig
  if (shockMult < 1 && this.monster) {
    const savedDmg = this.monster.dmg;
    this.monster.dmg = Math.floor(this.monster.dmg * shockMult);
    _origMonsterAtk();
    this.monster.dmg = savedDmg;
  } else {
    _origMonsterAtk();
  }

  if (this.monster) {
    this.addLog(`${this.monster.name} atacou ${this.heroName}`, "text-red-300");
  }
};

// Patch killMonster para diálogo de derrota
const _origKillMonster = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  this._bossHalfTriggered = false;
  this._bossLowTriggered = false;
  if (this.isBossFight) this.showBossDialogue("defeat");
  this.statusEffects = {};
  this.renderStatusBar();
  _origKillMonster();
};
...
rpg.save = function() {
  localStorage.setItem("rpg_lore_fragments", JSON.stringify(this.loreFragments || []));
  _origSave();
};

const _origInit = rpg.init.bind(rpg);
rpg.init = function() {
  this.loreFragments = JSON.parse(localStorage.getItem("rpg_lore_fragments") || "[]");
  _origInit();
};
...
const _killPatch = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  this.checkLoreUnlock();
  _killPatch();
};
...
const _notSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _notSpawn();
  if (this.monster) {
    const m = this.getNotorietyTier().enemyMult;
...
const _notKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _notKill();
  this.addNotoriety(this.isBossFight ? 50 : 1);
};
...
const _wxSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _wxSpawn();
  if (!this.isBossFight) this.rollWeather();
  if (this.currentWeather && this.monster) this.monster.dmg = Math.floor(this.monster.dmg * this.currentWeather.dmgMod);
};
const _wxDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(b, t, u) {
  _wxDmg(b, t, u);
...
const _comboSkill = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) { _comboSkill(id); if (this.inCombat) this.checkSkillCombo(id); };
...
const _repKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _repKill();
  const cls = this.eqClass, prev = this.getClassRepTier(cls);
  ...
  const next = this.getClassRepTier(cls);
  if (prev.bonus !== next.bonus) {
    const c = this.classes[cls]; showToast("⭐ "+( c ? c.name[this.lang] : cls)+" → "+next.label[this.lang]+"! +"+Math.round(next.bonus*100)+"% stats!", 4000);
  }
};
const _repAtk = rpg.getAtk.bind(rpg); rpg.getAtk = function() { return Math.floor(_repAtk()*(1+this.getClassRepBonus())); };
const _repHp  = rpg.getMaxHp.bind(rpg); rpg.getMaxHp = function() { return Math.floor(_repHp()*(1+this.getClassRepBonus())); };
...
const _ngSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _ngSpawn();
  if ((this.ngPlusActive||0) > 0 && this.monster) {
    const m = this.NG_ENEMY_MULT[this.ngPlusActive]||1;
    this.monster.hp = Math.floor(this.monster.hp*m); this.monster.maxHp = Math.floor(this.monster.maxHp*m); this.monster.dmg = Math.floor(this.monster.dmg*m);
  }
};
const _ngKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  const g0 = this.gold, x0 = this.xp;
  _ngKill();
  if ((this.ngPlusActive||0) > 0) {
    const r = (this.NG_REWARD_MULT[this.ngPlusActive]||1)-1;
    if (this.gold > g0) this.gold += Math.floor((this.gold-g0)*r);
    if (this.xp > x0)   this.xp   += Math.floor((this.xp-x0)*r);
  }
};
...
// Apply aura effects
const _auraMonsterAtk = rpg.executeMonsterAttack.bind(rpg);
rpg.executeMonsterAttack = function() {
  const a = this.getAura();
  if (a?.effect === "fire") setTimeout(()=>this.applyStatus("burn"), 100);
  _auraMonsterAtk();
};
const _auraDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(b, t, u) {
  const a = this.getAura();
  let dmg = b;
  if (a?.effect === "thunder" && this.fury >= 50) dmg = Math.floor(dmg * 1.20);
  if (a?.effect === "void") dmg = Math.floor(dmg * 1.40);
  if (a?.effect === "chaos" && t === "atk" && Math.random() < 0.15) {
    const statuses = ["burn","freeze","shock","bleed","stun"];
    this.applyStatus(statuses[Math.floor(Math.random()*statuses.length)]);
  }
  _auraDmg(dmg, t, u);
  if (a?.effect === "ice" && this.monster && (this.monster.hp/this.monster.maxHp) <= 0.25) this.applyStatus("freeze");
};
const _auraKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _auraKill();
  const a = this.getAura();
  if (a?.effect === "shadow") this.addFury(5);
};
const _auraHeal = rpg.useSkill.bind(rpg);
rpg.useSkill = function(id) {
  _auraHeal(id);
  if (id === "heal" && this.getAura()?.effect === "holy") {
    ...
    this.showDamage("✨ +"+formatNumber(bonus),"heal");
    this.updateHpBars();
  }
};
const _auraVoidHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  const base = _auraVoidHp();
  if (this.getAura()?.effect === "void") return Math.floor(base * 1.40);
  return base;
};
const _auraVoidSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _auraVoidSpawn();
  if (this.getAura()?.effect === "void" && this.monster) {
    this.monster.hp = Math.floor(this.monster.hp*1.20); this.monster.maxHp = Math.floor(this.monster.maxHp*1.20);
    this.monster.dmg = Math.floor(this.monster.dmg*1.20);
  }
};
...
const _mutKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _mutKill();
  if (this.prestigeLevel > _lastPrestigeLevel) { _lastPrestigeLevel = this.prestigeLevel; setTimeout(()=>this.gainMutation(),1500); }
};
// Mutation chaos: random stat variation per battle
const _mutChaosSpawn = rpg.spawnMonster.bind(rpg);
rpg.spawnMonster = function() {
  _mutChaosSpawn();
  if (this.mutChaosActive) {
    this._chaosAtkMod = 0.80 + Math.random()*0.40; // 0.8–1.2
    this._chaosHpMod  = 0.80 + Math.random()*0.40;
  } else { this._chaosAtkMod = 1; this._chaosHpMod = 1; }
};
const _mutAtkPatch = rpg.getAtk.bind(rpg);
rpg.getAtk = function() { return Math.floor(_mutAtkPatch()*(this._chaosAtkMod||1)*(this.mutBerserkerActive&&this.heroHp>0&&this.heroHp/Math.max(1,this.getMaxHp())<0.30?1.50:1)); };
// Mutation vampiric on kill
const _mutVampDmg = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(b,t,u) {
  _mutVampDmg(b,t,u);
  if ((this.permVampBonus||0)>0 && b>0) {
    const steal=Math.floor(b*this.permVampBonus);
    if(steal>0){this.heroHp=Math.min(this.getMaxHp(),this.heroHp+steal);}
  }
};
// Mutation phoenix
const _mutDie = rpg.die.bind(rpg);
rpg.die = function() {
  if ((this.mutPhoenixCount||0) > 0 && !this._mutPhoenixBattleUsed) {
    this._mutPhoenixBattleUsed = true; this.mutPhoenixCount--;
    this.heroHp = Math.floor(this.getMaxHp()*0.25);
    this.updateHpBars(); this.showDamage("🔥 FÉNIX!","heal"); return;
  }
  _mutDie();
};
const _mutConfirmBattle = rpg.confirmBattle.bind(rpg);
rpg.confirmBattle = function() { this._mutPhoenixBattleUsed = false; _mutConfirmBattle(); };
...
const _srKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  const wasSpeedRun = this.speedRunActive;
  const bossId = this.monster?.id;
  _srKill();
  if (wasSpeedRun && this.isBossFight && bossId) {
    clearInterval(this._srInterval);
    const elapsed = Date.now() - this.speedRunStart;
    const prev = this.speedRunBest[bossId];
    const isRecord = !prev || elapsed < prev;
    if (isRecord) this.speedRunBest[bossId] = elapsed;
    localStorage.setItem("rpg_sr_best", JSON.stringify(this.speedRunBest));
    // Reward: bonus gold based on time (faster = more)
    const bonusGold = Math.max(1000, Math.floor(this.level * 500 * (isRecord ? 3 : 1)));
    const bonusHonor = Math.max(10, Math.floor(60 - elapsed/1000));
    this.gold += bonusGold; this.addHonor(Math.max(1,bonusHonor));
    showToast((isRecord?"🏆 NOVO RECORD! ":"⏱ ")+ this._formatTime(elapsed)+" +" +formatNumber(bonusGold)+" 💰", 5000);
...
// Track deaths
const _secretDie = rpg.die.bind(rpg);
rpg.die = function() {
  this.totalDeaths = (this.totalDeaths||0)+1;
  localStorage.setItem("rpg_deaths", this.totalDeaths);
  this.checkSecretClasses();
  _secretDie();
};
...
const _v22Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_notoriety",       this.notoriety);
  localStorage.setItem("rpg_class_rep",       JSON.stringify(this.classReputation));
  localStorage.setItem("rpg_ng_plus",         this.ngPlusActive||0);
  localStorage.setItem("rpg_aura",            this.equippedAura||"");
  localStorage.setItem("rpg_auras_owned",     JSON.stringify(this.unlockedAuras));
  localStorage.setItem("rpg_mutations",       JSON.stringify(this.mutations));
  localStorage.setItem("rpg_sr_best",         JSON.stringify(this.speedRunBest));
  localStorage.setItem("rpg_oracle",          this.oracleConsults||0);
  localStorage.setItem("rpg_legendaries",     JSON.stringify(this.legendaryItems));
  localStorage.setItem("rpg_secret_classes",  JSON.stringify(this.secretClassesUnlocked));
  localStorage.setItem("rpg_deaths",          this.totalDeaths||0);
  localStorage.setItem("rpg_math_correct",    this.mathCorrectAnswers||0);
  _v22Save();
};

const _v22Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.notoriety          = parseInt(localStorage.getItem("rpg_notoriety")      || "0");
  this.classReputation    = JSON.parse(localStorage.getItem("rpg_class_rep")    || "{}");
  this.ngPlusActive       = parseInt(localStorage.getItem("rpg_ng_plus")        || "0");
  this.equippedAura       = localStorage.getItem("rpg_aura") || null;
  this.unlockedAuras      = JSON.parse(localStorage.getItem("rpg_auras_owned")  || "[]");
  this.mutations          = JSON.parse(localStorage.getItem("rpg_mutations")    || "[]");
  this.speedRunBest       = JSON.parse(localStorage.getItem("rpg_sr_best")      || "{}");
  this.oracleConsults     = parseInt(localStorage.getItem("rpg_oracle")         || "0");
  this.legendaryItems     = JSON.parse(localStorage.getItem("rpg_legendaries")  || "[]");
  this.secretClassesUnlocked = JSON.parse(localStorage.getItem("rpg_secret_classes") || "[]");
  this.totalDeaths        = parseInt(localStorage.getItem("rpg_deaths")         || "0");
  this.mathCorrectAnswers = parseInt(localStorage.getItem("rpg_math_correct")   || "0");
  // Init flags
  this.currentWeather = null; this.skillSequence = [];
  this.speedRunActive = false; this.speedRunTimer = 0; this._srInterval = null;
  this.permVampBonus = 0; this.permDefBonus = 0;
  this.mutChaosActive = false; this.mutBerserkerActive = false;
  this.mutPhoenixCount = 0; this.mutOracleActive = false;
  this._chaosAtkMod = 1; this._chaosHpMod = 1;
  this.immuneStun = false;
  // Apply mutations
  this._applyMutations();
  // Register secret classes in class list
  this.secretClassesUnlocked.forEach(scId => {
    const sc = this.SECRET_CLASS_DEFS.find(s=>s.id===scId);
    if (sc && !this.classes[scId]) this.classes[scId] = sc.class;
  });
  // Apply legendary items
  this.legendaryItems.forEach(id => {
    const r = this.LEGENDARY_RECIPES.find(x=>x.id===id);
    if (r) r.apply(this);
  });
  _lastPrestigeLevel = this.prestigeLevel || 0;
  _v22Init();
};
...
rpg.getFormation = function() { return this.FORMATIONS.find(f=>f.id===this.activeFormation) || this.FORMATIONS[0]; };
rpg.setFormation = function(id) {
  this.activeFormation = id;
  localStorage.setItem("rpg_formation", id);
  const f = this.getFormation();
  showToast("⚔ Formação: "+f.name[this.lang]+"!", 2000);
  this.renderFormationModal();
};
...
rpg.getAtk = function() {
  let base = _formGetAtk();
  const f = this.getFormation();
  if (f.id === "chaos") return Math.floor(base * (0.5 + Math.random()));
  return Math.floor(base * f.atkMod);
};
const _formGetMaxHp = rpg.getMaxHp.bind(rpg);
rpg.getMaxHp = function() {
  let base = _formGetMaxHp();
  const f = this.getFormation();
  if (f.id === "chaos") return Math.floor(base * (0.5 + Math.random()));
  return Math.floor(base * f.hpMod);
};
const _formGetCrit = rpg.getCritChance.bind(rpg);
rpg.getCritChance = function() { return Math.min(0.99, _formGetCrit() + (this.getFormation().critMod||0)); };
const _formGetDodge = rpg.getDodgeChance.bind(rpg);
rpg.getDodgeChance = function() { return Math.min(0.90, _formGetDodge() + (this.getFormation().dodgeMod||0)); };
...
rpg.dealDamageToMonster = function(b, t, u) {
  const f = this.getFormation();
  const dmg = (t === "mag" && f.magMod) ? Math.floor(b * f.magMod) : b;
  _formDealDmg(dmg, t, u);
};
// Chaos: reward bonus
const _formKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  const g0 = this.gold;
  _formKill();
  if (this.activeFormation === "chaos" && this.gold > g0) {
    this.gold += (this.gold - g0); // double gold
  }
};
...
const _legacyInit = rpg.init.bind(rpg);
rpg.init = function() {
  this.legacyItems = JSON.parse(localStorage.getItem("rpg_legacy") || "[]");
  this.activeFormation = localStorage.getItem("rpg_formation") || "balanced";
  this.applyLegacyItems();
  _legacyInit();
};
...
const _tourneyDmgPatch = rpg.dealDamageToMonster.bind(rpg);
rpg.dealDamageToMonster = function(b, t, u) {
  _tourneyDmgPatch(b, t, u);
  if (this.tourneyDone && this.isBossFight && b > this.tourneyBestDmg) {
    this.tourneyBestDmg = b;
    localStorage.setItem("rpg_tourney_dmg", b);
  }
};
...
const _memKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _memKill();
  if (this.isBossFight) this.checkSkillMemories();
};
...
const _hudUpdateTheme = rpg.updateTheme.bind(rpg);
rpg.updateTheme = function() { _hudUpdateTheme(); setTimeout(() => this.applyHudTheme(), 80); };
...
const _seasonKill = rpg.killMonster.bind(rpg);
rpg.killMonster = function() {
  _seasonKill();
  const pts = this.isBossFight ? 50 : 1;
  this.seasonKills = (this.seasonKills||0) + (this.isBossFight ? 0 : 1);
  localStorage.setItem("rpg_season_kills", this.seasonKills);
  this.addSeasonPoints(pts);
};
...
const _v23Save = rpg.save.bind(rpg);
rpg.save = function() {
  localStorage.setItem("rpg_formation",    this.activeFormation || "balanced");
  localStorage.setItem("rpg_legacy",       JSON.stringify(this.legacyItems||[]));
  localStorage.setItem("rpg_tourney_dmg",  this.tourneyBestDmg||0);
  localStorage.setItem("rpg_tourney_week", this.tourneyWeek||"");
  localStorage.setItem("rpg_tourney_done", this.tourneyDone?"true":"false");
  localStorage.setItem("rpg_memories",     JSON.stringify(this.skillMemories||[]));
  localStorage.setItem("rpg_season_pts",   this.seasonPoints||0);
  localStorage.setItem("rpg_season_kills", this.seasonKills||0);
  _v23Save();
};

const _v23Init = rpg.init.bind(rpg);
rpg.init = function() {
  this.activeFormation  = localStorage.getItem("rpg_formation") || "balanced";
  this.legacyItems      = JSON.parse(localStorage.getItem("rpg_legacy")       || "[]");
  this.tourneyBestDmg   = parseInt(localStorage.getItem("rpg_tourney_dmg")    || "0");
  this.tourneyWeek      = localStorage.getItem("rpg_tourney_week")             || "";
  this.tourneyDone      = localStorage.getItem("rpg_tourney_done") === "true";
  this.skillMemories    = JSON.parse(localStorage.getItem("rpg_memories")      || "[]");
  this.seasonPoints     = parseInt(localStorage.getItem("rpg_season_pts")      || "0");
  this.seasonKills      = parseInt(localStorage.getItem("rpg_season_kills")    || "0");
  // Apply skill memories
  this.SKILL_MEMORIES.forEach(mem => { if (this.skillMemories.includes(mem.id)) mem.apply(this); });
  _v23Init();
};
