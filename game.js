...
rpg.toggleAutoAttack = function () {
  ...
  if (this.autoAttack) {
    ...
    showToast(t("auto_on"));
    // Se já estamos em combate mas o loop parou, relançar
    if (this.inCombat && !this.isSpawning && this.monster && this.monster.hp > 0 && this.heroHp > 0) {
...
rpg.startBattle = function (isBoss) {
  ...
  if (autoBtn) { autoBtn.disabled = false; autoBtn.style.pointerEvents = "auto"; autoBtn.style.opacity = "1"; autoBtn.style.filter = "none"; }
  document
...
rpg.combatTick = function (timestamp) {
  ...
  // Only reschedule if still in active combat
  if (this.inCombat && this.heroHp > 0 && !this.isSpawning) {
    this.combatFrame = requestAnimationFrame(this.combatTick.bind(this));
  }
};
...
rpg.startCooldown = function (id) {
  ...
  if (_autoBtn) { _autoBtn.disabled = false; _autoBtn.style.opacity = "1"; _autoBtn.style.filter = "none"; }
...
rpg.MAP_REGIONS = [
  ...
  {
    id: "r_quantum",    x: 50, y: 8,  name: {pt:"Núcleo Quântico",       en:"Quantum Core"},
  ...
  {
    id: "r_neural",     x: 75, y: 8,  name: {pt:"Rede Neural",           en:"Neural Network"},
    ...
    theme: "t_neural", chapter: "cap_xi", biome: "neural"
  },
  {
    id: "r_end",        x: 50, y: 18, name: {pt:"O Fim de Tudo",         en:"The End of All"},
...
rpg.renderMap = function() {
  ...
  const connections = [
    ...
    ["r_quantum","r_neural"], ["r_quantum","r_end"], ["r_neural","r_end"],
  ];
...
// ═══════════════════════════════════════════════════════════════
// ── CONSOLIDATED save / init / startCooldown HOOKS ───────────
// ═══════════════════════════════════════════════════════════════

const _ORIG_save = rpg.save.bind(rpg);
const _ORIG_init = rpg.init.bind(rpg);
const _ORIG_startCooldown = rpg.startCooldown.bind(rpg);
rpg.save = function() {
  _ORIG_save();
  // v17
  ...
  localStorage.setItem("rpg_grimoire",          JSON.stringify(this.grimoire||[]));
  localStorage.setItem("rpg_best_wave",         this.bestWave||0);
  localStorage.setItem("rpg_dungeons",          this.dungeonsCleared||0);
  localStorage.setItem("rpg_total_gold",        this.totalGoldEarned||0);
  localStorage.setItem("rpg_dd_date",           this.dailyDungeonDate||"");
  localStorage.setItem("rpg_dd_done",           this.dailyDungeonDone?"true":"false");
  // v19
  localStorage.setItem("rpg_talent_pts",        this.talentPoints||0);
  localStorage.setItem("rpg_talents",           JSON.stringify(this.unlockedTalents||[]));
  localStorage.setItem("rpg_forge",             JSON.stringify(this.forgeUpgrades||{}));
  localStorage.setItem("rpg_gems",              JSON.stringify(this.equippedGems||{}));
  localStorage.setItem("rpg_owned_gems",        JSON.stringify(this.ownedGems||[]));
  localStorage.setItem("rpg_cursed_relic",      this.equippedCursedRelic||"");
  localStorage.setItem("rpg_cursed_relics",     JSON.stringify(this.unlockedCursedRelics||[]));
  localStorage.setItem("rpg_honor",             this.honor||0);
  localStorage.setItem("rpg_honor_shop",        JSON.stringify(this.purchasedHonor||[]));
  // v20
  localStorage.setItem("rpg_map",              JSON.stringify(this.mapDiscovered||[]));
  ...
  localStorage.setItem("rpg_narrative",        JSON.stringify(this.narrativeChoices||{}));
  localStorage.setItem("rpg_npc_quests",       JSON.stringify(this.npcQuestsDone||[]));
  localStorage.setItem("rpg_ult_uses",         this.ultimateUses||0);
  localStorage.setItem("rpg_hardcore_wins",    this.hardcoreWins||0);
  // v21
  localStorage.setItem("rpg_diary",            JSON.stringify(this.heroDiary||[]));
  localStorage.setItem("rpg_proc_best",        this.procDungeonBestFloor||0);
  // v22
  localStorage.setItem("rpg_notoriety",        this.notoriety||0);
  localStorage.setItem("rpg_class_rep",        JSON.stringify(this.classReputation||{}));
...
rpg.init = function() {
  ...
  this.procDungeonBestFloor = parseInt(localStorage.getItem("rpg_proc_best")           ||"0");
  this.proceduralDungeon    = null; this.procDungeonFloor=0; this.procDungeonActive=false;
  // v22
  ...
  this.runeKillStack=0;
  this.waveNumber=1; this.waveActive=false;
  this.dailyDungeonActive=false; this.mathBattleActive=false; this.mathBonusActive=false;
  this.challengeActive=false; this.activeChallenges=[];
  ...
  this.BOSS_PARTS_ACTIVE=false; this.bossPartHP={}; this.targetedPart=null;
  this.lastElement=null; this.statusEffects={};
  this.grimoirePhoenix=false; this.grimoireFuryBoost=false;
  this.grimoireLifeSteal=false; this.grimoireComboMax=20; this.grimoireArcaneSurge=false;
  ...
  // Apply saved systems
  this._applyAchievements && this._applyAchievements();
  this._applyGrimoire     && this._applyGrimoire();
  this._applyTalents      && this._applyTalents();
  this._applyMutations    && this._applyMutations();

  // Apply narrative choices
  Object.entries(this.narrativeChoices||{}).forEach(([cid,optId])=>{
    const ch=(this.NARRATIVE_CHOICES||[]).find(c=>c.id===cid);
    if(ch){const opt=ch.options.find(o=>o.id===optId); if(opt) opt.apply(this);}
  });
  // Apply NPC quest rewards
  (this.npcQuestsDone||[]).forEach(qid=>{
    (this.NPCS||[]).forEach(npc=>{const q=npc.quests.find(x=>x.id===qid); if(q) q.apply(this);});
  });
  // Apply legendary items
  (this.legendaryItems||[]).forEach(id=>{
    const r=(this.LEGENDARY_RECIPES||[]).find(x=>x.id===id); if(r) r.apply(this);
  });
  // Apply honor shop
  (this.purchasedHonor||[]).forEach(id=>{
    const item=(this.HONOR_SHOP||[]).find(h=>h.id===id); if(item?.apply) item.apply(this);
  });
  // Register secret classes
  ...
  // Apply legacy items
  this.applyLegacyItems && this.applyLegacyItems();
  // Discover starting region
  if(!(this.mapDiscovered||[]).includes("r_ruins")) (this.mapDiscovered=this.mapDiscovered||[]).push("r_ruins");
...
rpg.startCooldown = function(id) {
  const skill = this.skills[id];
  if (!skill) { _ORIG_startCooldown(id); return; }
  const orig = skill.cd;
  // Relic: capacitor
  if (this.inventory.includes("r_capacitor")) skill.cd = Math.floor(orig * 0.5);
  else if (this.inventory.includes("r_time")) skill.cd = Math.floor(orig * 0.8);
  // Talent CD reduction
  if ((this.talentCdReduce||0) > 0) skill.cd = Math.floor(skill.cd * (1 - this.talentCdReduce));
  // Temp CD reduction (wanderer scroll)
  if ((this.tempCdBoost||0) > 0) skill.cd = Math.floor(skill.cd * (1 - this.tempCdBoost));
  // Timeloop rune: 20% free - skip cooldown but still briefly disable btn
  if (this.getRuneEffect?.("timeloop") && Math.random()<0.20) {
    skill.cd = orig;
    // Brief visual flash only, no real CD
    skill.timer = true;
...
// ═══════════════════════════════════════════════════════════════
// ── GLOBAL OPEN FUNCTIONS (called from HTML buttons) ─────────
// ═══════════════════════════════════════════════════════════════

function openAchievements()  { rpg.renderAchievements();   document.getElementById('achievements-modal').classList.add('active'); }
function openArena()         { rpg.renderArena();           document.getElementById('arena-modal').classList.add('active'); }
function openAuras()         { rpg.renderAuraModal();       document.getElementById('aura-modal').classList.add('active'); }
function openBattlePass()    { rpg.renderBattlePass();      document.getElementById('battlepass-modal').classList.add('active'); }
function openChallenge()     { rpg.renderChallengeModal();  document.getElementById('challenge-modal').classList.add('active'); }
function openCoop()          { rpg.renderCoop();            document.getElementById('coop-modal').classList.add('active'); }
function openCraft()         { rpg.renderCraftModal();      document.getElementById('craft-modal').classList.add('active'); }
function openCursedRelics()  { rpg.renderCursedRelics();    document.getElementById('cursed-modal').classList.add('active'); }
function openDaily()         { rpg.renderDailyMissions();  document.getElementById('daily-modal').classList.add('active'); }
function openDailyDungeon()  { rpg.renderDailyDungeon();    document.getElementById('dd-modal').classList.add('active'); }
function openDiary()         { rpg.renderDiary();           document.getElementById('diary-modal').classList.add('active'); }
...
function openNgPlus()        { rpg.renderNgPlus();          document.getElementById('ngplus-modal').classList.add('active'); }
